...
read_initial_seed_lp:
  long rangeStart = 0;
  long rangeLen = 0;
  scanfResult = fscanf(pInput, "%ld %ld", &rangeStart, &rangeLen);
  {
    void *aJumpTable[] = {&&end_read_initial_seed_lp, &&expand_range};
    goto *aJumpTable[scanfResult == 2];
  }
expand_range:
  aInitialSeeds[initialSeedCount++] = rangeStart++;
  --rangeLen;
  {
    void *aJumpTable[] = {&&expand_range, &&read_initial_seed_lp};
    goto *aJumpTable[rangeLen == 0];
  }
