...
read_next_input_000:
  int scanfResult = sscanf(pLine + offset, "%ld%n", &aSeq[0][seqLen++], &charsRead);
  {
...
populate_next_cell:
  ...
  ++workRowIdx;
  {
    ...
    goto *aJump[workRowIdx < workRowLen];
  }
...
extrapolate:
  --workRowNum;
  ++workRowIdx;
  aSeq[workRowNum][workRowIdx] = aSeq[workRowNum][workRowIdx - 1] + aSeq[workRowNum + 1][workRowIdx - 1];
  {
...
extrapolation_complete:
  totalExtrapolated += aSeq[workRowNum][workRowIdx];
