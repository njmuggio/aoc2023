...
int main(int argc, char **pArgv)
{
  ...
  unsigned char aColOffsets[140] = {0};
  unsigned char aRowOffsets[140] = {0};
...
inflate_row_start:
  int curRow = 1;
inflate_row_100:
  aRowOffsets[curRow] = aRowOffsets[curRow - 1] + aEmptyRows[curRow];
  ++curRow;
  {
...
inflate_col_start:
  int curCol = 1;
inflate_col_100:
  aColOffsets[curCol] = aColOffsets[curCol - 1] + aEmptyCols[curCol];
  ++curCol;
  {
...
inflate_col_in_lp_000:
  int inflatedRowOffset = abs((aLocs[locAIdx].y + aRowOffsets[aLocs[locAIdx].y]) - (aLocs[locBIdx].y + aRowOffsets[aLocs[locBIdx].y]));
  int inflatedColOffset = abs((aLocs[locAIdx].x + aColOffsets[aLocs[locAIdx].x]) - (aLocs[locBIdx].x + aColOffsets[aLocs[locBIdx].x]));
